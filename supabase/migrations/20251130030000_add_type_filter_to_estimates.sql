-- Drop old versions to be safe, though CREATE OR REPLACE handles it if signature matches.
-- Since signature changes (adding argument), we need to be careful.
...
BEGIN
    ...
    query_str := format('
      ...
      WHERE
          ...
          (%4$L IS NULL OR %4$L = '''' OR
              e."projectName" ILIKE %5$L OR
              e.type ILIKE %5$L OR
              e."serviceType" ILIKE %5$L OR
              c.name ILIKE %5$L OR
              c.email ILIKE %5$L
          )
      ORDER BY %6$I %7$s',
